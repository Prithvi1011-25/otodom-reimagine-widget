...
let tunnelOriginCache = null;
let tunnelOriginPromise = null;
...
async function fetchTunnelOrigin() {
  if (tunnelOriginCache) return tunnelOriginCache;
  if (tunnelOriginPromise) return tunnelOriginPromise;

  tunnelOriginPromise = fetch('/__reih_public_origin')
    .then((response) => response.json())
    .then((data) => {
      tunnelOriginCache = typeof data?.origin === 'string' ? data.origin : '';
      return tunnelOriginCache;
    })
    .catch(() => '');

  return tunnelOriginPromise;
}
